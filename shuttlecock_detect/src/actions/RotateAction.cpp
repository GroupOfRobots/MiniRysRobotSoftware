...
bool RotateAction::setGoal(RosActionNode::Goal &goal) { return true; }

NodeStatus
RotateAction::onResultReceived(const RosActionNode::WrappedResult &wr) {
...
NodeStatus RotateAction::onFailure(ActionNodeErrorCode error) {
  RCLCPP_ERROR(logger(), "%s: onFailure with error: %s", name().c_str(),
               toStr(error));
