...
Distances::Distances(rclcpp::NodeOptions options) : Node("distances", options) {
  ...
  this->declare_parameter("weights",
                          rclcpp::ParameterValue("best100ep1inp.ncnn"));
  ...
  this->declare_parameter("ccd", rclcpp::ParameterValue(3.6));

  // load parameters
  std::string package_share_directory =
      ament_index_cpp::get_package_share_directory("shuttlecock_detect");
  ...
  ccd_ = (float)this->get_parameter("ccd").as_double();
  ...
  yolov7_->load(target_size,
                package_share_directory + "/weights/" + weight + ".param",
                package_share_directory + "/weights/" + weight + ".bin");
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: prob_threshold " << prob_threshold_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: nms_threshold " << nms_threshold_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: target_size " << target_size);
  RCLCPP_INFO_STREAM(this->get_logger(), "Got param: weight " << weight);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: width_front " << width_front_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: width_side " << width_side_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: focal_length " << focal_length_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: timer_period " << timer_period);
  RCLCPP_INFO_STREAM(this->get_logger(), "Got param: ccd " << ccd_);

  // publishers
  publisher_dat_ =
      this->create_publisher<btcpp_ros2_interfaces::msg::DistancesAndTransform>(
          "distances", 10);
  timer_ = this->create_wall_timer(std::chrono::duration<double>(timer_period),
                                   std::bind(&Distances::timer_callback, this));
  publisher_detected_ =
  ...
  subscription_image_ = this->create_subscription<sensor_msgs::msg::Image>(
      "/cv_video_frames_plain_img", 10,
      std::bind(&Distances::image_callback, this, std::placeholders::_1));
...
void Distances::timer_callback() {
  ...
  if (!ori_img_.empty()) {
    try {
      msg.transform = tf_buffer_->lookupTransform(
          "minirys2/map", "minirys2/base_link", tf2::TimePointZero);
...
std::pair<float, float> Distances::calculate_dist() {
  ...
  auto msg_img =
      cv_bridge::CvImage(std_msgs::msg::Header(), "rgb8", detected_img_)
          .toImageMsg();
  publisher_detected_->publish(*msg_img);
  float dist, deltX;
  float focal_px = focal_length_ * (float)detected_img_.size().width / ccd_;
  if (objects.size() > 0) {
    dist = ((objects[0].label == 0 ? width_side_ : width_front_) * focal_px) /
           objects[0].rect.width;
    deltX = dist / focal_length_ * (ccd_ / detected_img_.size().width) *
            ((float)objects[0].rect.x + (float)objects[0].rect.width / 2.0 -
             (float)detected_img_.size().width / 2.0);
