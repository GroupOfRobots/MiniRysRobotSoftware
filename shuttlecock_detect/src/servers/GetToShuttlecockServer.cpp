...
GetToShuttlecockServer::GetToShuttlecockServer(
    const rclcpp::NodeOptions &options)
    : Node("get_to_shuttlecock_action_server", options) {
  using namespace std::placeholders;

  this->action_server_ = rclcpp_action::create_server<Standard>(
      this, "get_to_shuttlecock_service",
      std::bind(&GetToShuttlecockServer::handle_goal, this, _1, _2),
  ...
  stop_from_planner_ =
      (float)this->get_parameter("stop_from_planner").as_double();
  timer_period_ = this->get_parameter("timer_period").as_double();
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: stop_from_planner " << stop_from_planner_);
  RCLCPP_INFO_STREAM(this->get_logger(),
  ...
  publisher_isCoverage_ =
      this->create_publisher<std_msgs::msg::Bool>("coverage", 10);

  // subscribers
  subscription_dat_ = this->create_subscription<
      btcpp_ros2_interfaces::msg::DistancesAndTransform>(
      "distances", 10,
      std::bind(&GetToShuttlecockServer::distance_callback, this,
                std::placeholders::_1));

  publisher_cancel_ =
      this->create_publisher<std_msgs::msg::Bool>("stop_navigate", 10);

  distance_ = -1.0;
}

rclcpp_action::GoalResponse GetToShuttlecockServer::handle_goal(
...
rclcpp_action::CancelResponse GetToShuttlecockServer::handle_cancel(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
  RCLCPP_INFO(this->get_logger(), "Received request to cancel goal");
  (void)goal_handle;
  return rclcpp_action::CancelResponse::ACCEPT;
}

void GetToShuttlecockServer::handle_accepted(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
  using namespace std::placeholders;
  std::thread{std::bind(&GetToShuttlecockServer::execute, this, _1),
              goal_handle}
      .detach();
}

void GetToShuttlecockServer::execute(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
  ...
  while (rclcpp::ok()) {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds((int)(timer_period_ * 1000.0)));
  }
}

void GetToShuttlecockServer::distance_callback(
    const btcpp_ros2_interfaces::msg::DistancesAndTransform::SharedPtr msg) {
...
void GetToShuttlecockServer::send_goal() {
  tf2::Quaternion q(
      transform_.transform.rotation.x, transform_.transform.rotation.y,
      transform_.transform.rotation.z, transform_.transform.rotation.w);
