...
SearchingShuttlecockServer::SearchingShuttlecockServer(
    const rclcpp::NodeOptions &options)
    : Node("searching_shuttlecock_action_server", options) {
  using namespace std::placeholders;

  this->action_server_ = rclcpp_action::create_server<Standard>(
      this, "searching_shuttlecock_service",
      std::bind(&SearchingShuttlecockServer::handle_goal, this, _1, _2),
  ...
  subscription_dat_ = this->create_subscription<
      ...
      "distances", 10,
      std::bind(&SearchingShuttlecockServer::distance_callback, this,
                std::placeholders::_1));

  distance_ = -1.0;
}

rclcpp_action::GoalResponse SearchingShuttlecockServer::handle_goal(
    const rclcpp_action::GoalUUID &,
    std::shared_ptr<const Standard::Goal> goal) {
  RCLCPP_INFO(this->get_logger(),
              "Received goal request searching shuttlecocks");
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse SearchingShuttlecockServer::handle_cancel(
...
void SearchingShuttlecockServer::handle_accepted(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
  using namespace std::placeholders;

  std::thread{std::bind(&SearchingShuttlecockServer::execute, this, _1),
              goal_handle}
      .detach();
}

void SearchingShuttlecockServer::execute(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
  ...
  while (rclcpp::ok()) {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds((int)(timer_period_ * 1000.0f)));
  }
...
void SearchingShuttlecockServer::distance_callback(
