...
PickShuttlecockServer::PickShuttlecockServer(const rclcpp::NodeOptions &options)
  ...
  this->action_server_ = rclcpp_action::create_server<Standard>(
      this, "pick_shuttlecock_service",
      std::bind(&PickShuttlecockServer::handle_goal, this, _1, _2),
  ...
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: linear speed " << linear_speed_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: stop_docking " << stop_docking_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: stop_from_planner " << stop_from_planner_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param:  how_many_trys " << how_many_trys_);
  ...
  subscription_dat_ = this->create_subscription<
      ...
      "distances", 10,
      std::bind(&PickShuttlecockServer::distance_callback, this,
                std::placeholders::_1));
...
PickShuttlecockServer::handle_goal(const rclcpp_action::GoalUUID &,
                                   std::shared_ptr<const Standard::Goal> goal) {
  RCLCPP_INFO(this->get_logger(), "Received goal request with sleep time ");
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse PickShuttlecockServer::handle_cancel(
...
void PickShuttlecockServer::handle_accepted(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
  using namespace std::placeholders;
  // this needs to return quickly to avoid blocking the executor, so spin up a
  // new thread
  std::thread{std::bind(&PickShuttlecockServer::execute, this, _1), goal_handle}
      .detach();
}

void PickShuttlecockServer::execute(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
...
void PickShuttlecockServer::distance_callback(
