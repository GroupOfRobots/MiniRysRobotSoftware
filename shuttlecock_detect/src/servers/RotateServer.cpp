...
RotateServer::RotateServer(const rclcpp::NodeOptions &options)
  ...
  this->action_server_ = rclcpp_action::create_server<Standard>(
      this, "rotate_service",
      std::bind(&RotateServer::handle_goal, this, _1, _2),
  ...
  stop_rotate_dist_ =
      (float)this->get_parameter("stop_rotate_dist").as_double();
  stop_rotate_ = (float)this->get_parameter("stop_rotate").as_double();
  this->pid_ = std::unique_ptr<PIDRegulator>(
      new PIDRegulator(timer_period_, (float)K, (float)Ti, (float)Td));
  ...
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: stop_rotate " << stop_rotate_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: timer_period " << timer_period_);
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: stop_rotate_dist " << stop_rotate_dist_);
  ...
  subscription_dat_ = this->create_subscription<
      ...
      "distances", 10,
      std::bind(&RotateServer::distance_callback, this, std::placeholders::_1));
...
void RotateServer::handle_accepted(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
  using namespace std::placeholders;
  // this needs to return quickly to avoid blocking the executor, so spin up a
  // new thread
  std::thread{std::bind(&RotateServer::execute, this, _1), goal_handle}
      .detach();
}

void RotateServer::execute(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
...
void RotateServer::distance_callback(
