...
BackUpServer::BackUpServer(const rclcpp::NodeOptions &options)
  ...
  this->action_server_ = rclcpp_action::create_server<Standard>(
      this, "back_up_service",
      std::bind(&BackUpServer::handle_goal, this, _1, _2),
  ...
  RCLCPP_INFO_STREAM(this->get_logger(),
                     "Got param: back_up_dist " << back_up_dist_);
...
BackUpServer::handle_goal(const rclcpp_action::GoalUUID &,
                          std::shared_ptr<const Standard::Goal> goal) {
  RCLCPP_INFO(this->get_logger(), "Received goal request");
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse BackUpServer::handle_cancel(
...
void BackUpServer::handle_accepted(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
  using namespace std::placeholders;
  std::thread{std::bind(&BackUpServer::execute, this, _1), goal_handle}
      .detach();
}

void BackUpServer::execute(
    const std::shared_ptr<GoalHandleStandard> goal_handle) {
