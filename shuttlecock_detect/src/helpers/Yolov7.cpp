...
int YoloV7::load(int _target_size, std::string weight_param,
  ...
  yolo.load_param(weight_param.c_str());
  yolo.load_model(weight_bin.c_str());
...
int YoloV7::detect(const cv::Mat &rgb, std::vector<Object> &objects,
...
int YoloV7::draw(cv::Mat &rgb, const std::vector<Object> &objects) {
  static const char *class_names[] = {"shuttlecock_back", "shuttlecock_front",
                                      "shuttlecock_side"};
